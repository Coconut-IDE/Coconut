 ...
 */
public class LocationManagerRequestLocationUpdate1AnnotationUtil extends PersonalDataAPIAnnotationUtil {
...
//        return CodeInspectionUtil.createEmptyAnnotationHolderByType(CoconutAnnotationType.LocationAnnotation);
        ...
        if (source == null || ((PsiMethodCallExpression)source).getArgumentList().getExpressions().length <= 2) {
            return annotationHolder;
        }
        PsiExpression provider = ((PsiMethodCallExpression)source).getArgumentList().getExpressions()[0];
        PsiExpression minTime = ((PsiMethodCallExpression) source).getArgumentList().getExpressions()[1];
        PsiExpression minDistance = ((PsiMethodCallExpression) source).getArgumentList().getExpressions()[2];
    ...
    public LocalQuickFix[] getModifyFieldValueAndCodeQuickfixList(PsiMethodCallExpression methodCallExpression, PsiNameValuePair nameValuePair, ArrayList<String> fieldValue) {
        ...
        localQuickFixes.add(new ModifyFieldValueAndCodeQuickfix("(Modify code) use NETWORK_PROVIDER (>100m accuracy)", methodCallExpression, nameValuePair, fieldValue,
                (callExpression)->LocationUtils.providerBasedChangeCodeFunction(0, "LocationManager.NETWORK_PROVIDER", callExpression)));
//        localQuickFixes.add(new ModifyFieldValueAndCodeQuickfix("test comment", methodCallExpression, nameValuePair, fieldValue,
//                (callExpression)->{
//                    PsiElementFactory factory = JavaPsiFacade.getInstance(methodCallExpression.getProject()).getElementFactory();
//                    PsiStatement methodCallStatementParent = PsiTreeUtil.getParentOfType(methodCallExpression, PsiStatement.class);
//                    PsiField methodCallFieldParent = PsiTreeUtil.getParentOfType(methodCallExpression, PsiField.class);
//                    if (methodCallStatementParent != null) {
//                        PsiComment commentedMethodCallStatementParent = factory.createCommentFromText("/*" + methodCallStatementParent.getText() + "*/", null);
//                        methodCallStatementParent.replace(commentedMethodCallStatementParent);
//                    } else if (methodCallFieldParent != null) {
//                        PsiComment commentedMethodCallFieldParent = factory.createCommentFromText("/*" + methodCallFieldParent.getText() + "*/", null);
//                        methodCallFieldParent.replace(commentedMethodCallFieldParent);
//                    }
//                }));
        return localQuickFixes.toArray(new LocalQuickFix[0]);
    }
